...
rem ===================================================================
rem Script Name   : stm32cubeidec_clean_build_bebug.bat
rem                 STM32CubeIDE headless Debug build batch file
rem
rem Description   : Automatically detects the STM32CubeIDE project,
rem                 imports it into a temporary workspace and performs
rem                 a clean Debug build using stm32cubeidec.exe.
rem
...
rem Change History:
rem -------------------------------------------------------------------
rem Version 2.1.0 | Aug 7, 2026 | Corrected project path.
...
rem Version 1.0.0 | Aug 4, 2026 | Initial debug.
rem ===================================================================
...
for /f "tokens=3 delims=<>" %%A in ('
    findstr /C:"<name>" "%PROJECT_DIR%\.project"
') do (
    set "PROJECT_NAME=%%A"
    goto :ProjectNameFound
)

@echo off
rem ===================================================================
rem Script Name : saleae_logic_run.bat
rem Saleae Logic launch batch file
rem
rem Description : Automatically verifies the existence of Saleae Logic,
rem               logs the process to the console, and launches the app.
rem
rem Location    : Any directory or Desktop.
rem
rem Usage       : Double-click the script to run Saleae Logic.
rem
...
echo.
echo %~n0 [INFO] Starting Saleae Logic script at %TIME% %DATE%
echo.

set "APP_PATH=C:\Program Files\Logic\Logic.exe"
echo APP_PATH = "%APP_PATH%"

rem Проверка существования файла
if not exist "%APP_PATH%" (
  echo [%DATE% %TIME%] [ERROR] Saleae Logic executable not found at:
  echo "%APP_PATH%"
...
echo [%DATE% %TIME%] [INFO] Launching Saleae Logic...
